/**
 * CaravelaSvelte composables — ergonomic access to the Phoenix hook context
 * from any Svelte component mounted by CaravelaSvelteHook, without prop drilling.
 *
 * Context architecture:
 *   The CaravelaSvelteHook passes live ref and connection state via Svelte's context
...
/** Context key for the Phoenix hook reference (`this` inside CaravelaSvelteHook). */
export const LIVE_SYMBOL = Symbol("__caravela_svelte__")

/**
 * Context key for the reactive connection state object.
 * The CaravelaSvelteHook sets this to `{ connected: boolean }` and updates it
 * via its `disconnected()` / `reconnected()` lifecycle callbacks.
 */
export const CONNECTION_SYMBOL = Symbol("__caravela_svelte_connection__")
...
/** Return type of useCaravelaSvelte(). */
export interface UseCaravelaSvelteResult {
...
/**
 * Access the Phoenix hook context from any Svelte component mounted by CaravelaSvelteHook.
 *
 * @throws {Error} When called outside a CaravelaSvelte-mounted component tree.
 *
 * @example
 * ```svelte
 * <script>
 *   import { useCaravelaSvelte } from "caravela_svelte"
 *   const { pushEvent } = useCaravelaSvelte()
 *   function save() { pushEvent("save", { value }) }
 * </script>
 * ```
 */
export function useCaravelaSvelte(): UseCaravelaSvelteResult {
  ...
  if (!live) {
    ...
    throw new Error("useCaravelaSvelte() must be called inside a CaravelaSvelte-mounted component")
  }
  return {
    get live() {
      return live
 ...
 * ```svelte
 * <script>
 *   import { useLiveEvent } from "caravela_svelte"
...
export function useLiveEvent(event: string, callback: (payload: unknown) => void): void {
  ...
  if (!live) {
    ...
    throw new Error("useLiveEvent() must be called inside a CaravelaSvelte-mounted component")
  }
 ...
 *
 * Connection state is managed by the CaravelaSvelteHook via `disconnected()` /
 * `reconnected()` lifecycle callbacks. Returns `{ connected: true }` when
 * called outside a CaravelaSvelte context (e.g. SSR, tests).
 *
 * @example
 * ```svelte
 * <script>
 *   import { useLiveConnection } from "caravela_svelte"
...
export function useLiveConnection(): UseLiveConnectionResult {
  ...
  if (!state) {
    // Outside CaravelaSvelte context — assume connected (SSR, unit testing)
 ...
 * ```svelte
 * <script>
 *   import { useLiveNavigation } from "caravela_svelte"
...
export function useLiveNavigation(): UseLiveNavigationResult {
  ...
  if (!live) {
    ...
    throw new Error("useLiveNavigation() must be called inside a CaravelaSvelte-mounted component")
  }
