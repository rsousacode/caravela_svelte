 ...
 *
 * Mocking strategy:
 *   - `svelte` module is mocked: getContext (for useCaravelaSvelte), onDestroy (capture cleanup).
...
// ---------------------------------------------------------------------------
// Graceful degradation without CaravelaSvelte context
// ---------------------------------------------------------------------------

describe("graceful degradation without CaravelaSvelte context", () => {
  beforeEach(() => {
  ...
  it("execute() returns a rejected promise when context is absent", async () => {
    const { execute } = useEventReply("compute")
    await expect(execute({})).rejects.toThrow("no CaravelaSvelte context")
  })
