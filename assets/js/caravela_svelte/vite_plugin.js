...
/**
 * CaravelaSvelte Vite plugin for SSR, hot reload support, and component auto-discovery.
 *
 * NOTE: Unlike LiveVue, CaravelaSvelte's `render()` function returns a `{head, html, css}`
 * object (not a plain HTML string). This plugin serialises that result as JSON so the
 * Elixir `CaravelaSvelte.SSR.ViteJS` module can decode it with `Jason.decode!/1`.
 *
        ...
        if (req.method == "POST" && req.url?.split("?", 1)[0] === ssrPath) {
          jsonMiddleware(req, res, async () => {
            try {
              ...
              // CaravelaSvelte render returns {head, html, css} — must JSON-encode for Elixir decoder
              res.setHeader("Content-Type", "application/json")
