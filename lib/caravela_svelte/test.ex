defmodule CaravelaSvelte.Test do
  @moduledoc """
  Helpers for testing CaravelaSvelte components and views.
  ...
  ## Examples
      ...
      svelte = CaravelaSvelte.Test.get_svelte(view)

      # Get by component name
      svelte = CaravelaSvelte.Test.get_svelte(view, name: "MyComponent")

      # Get by DOM id
      svelte = CaravelaSvelte.Test.get_svelte(view, id: "MyComponent-1")

      # From HTML string
      svelte = CaravelaSvelte.Test.get_svelte(html, name: "Counter")
  """
  ...
  Returns a map with:
    ...
    * `:handlers` - Event handlers (from `data-handlers` if present; currently %{} in CaravelaSvelte)
  ...
  def get_svelte(html, opts) when is_binary(html) do
    if Code.ensure_loaded?(LazyHTML) do
      lazy_html =
        ...
        |> LazyHTML.query("[phx-hook='CaravelaSvelteHook']")
      ...
      raise "LazyHTML is not installed. Add {:lazy_html, \">= 0.1.0\", only: :test} to your dependencies to use CaravelaSvelte.Test"
    end
  end
  ...
  # CaravelaSvelte does not emit data-handlers yet
