defprotocol CaravelaSvelte.Encoder do
  @moduledoc """
  Protocol for encoding values for CaravelaSvelte JSON serialization.
  ...
  * `@derive CaravelaSvelte.Encoder` — encode all struct fields except `:__struct__`
  * `@derive {CaravelaSvelte.Encoder, only: [:a, :b]}` — encode only listed keys
  * `@derive {CaravelaSvelte.Encoder, except: [:secret]}` — encode all except listed keys

  ## Example

      defmodule User do
        @derive {CaravelaSvelte.Encoder, except: [:password]}
...
defimpl CaravelaSvelte.Encoder, for: Integer do
  def encode(value, _opts), do: value
end

defimpl CaravelaSvelte.Encoder, for: Float do
  def encode(value, _opts), do: value
end

defimpl CaravelaSvelte.Encoder, for: BitString do
  def encode(value, _opts), do: value
end

defimpl CaravelaSvelte.Encoder, for: Atom do
  def encode(atom, _opts), do: atom
end

defimpl CaravelaSvelte.Encoder, for: List do
  def encode(list, opts) do
    Enum.map(list, &CaravelaSvelte.Encoder.encode(&1, opts))
  end
end

defimpl CaravelaSvelte.Encoder, for: Map do
  def encode(map, opts) do
    Map.new(map, fn {key, value} ->
      {key, CaravelaSvelte.Encoder.encode(value, opts)}
    end)
  end
end

defimpl CaravelaSvelte.Encoder, for: [Date, Time, NaiveDateTime, DateTime] do
  def encode(value, _opts) do
    @for.to_iso8601(value)
  end
end

defimpl CaravelaSvelte.Encoder, for: Phoenix.HTML.Form do
  ...
  if Code.ensure_loaded?(Ecto.Association.NotLoaded) do
    defp maybe_enhance_error(%{value: %Ecto.Association.NotLoaded{}} = error) do
      Map.update!(error, :description, fn description ->
        ...
        addition =
          "\n\nEncode form with CaravelaSvelte.Encoder.encode(form, nilify_not_loaded: true) to avoid."
  ...
  if Code.ensure_loaded?(Ecto.Changeset) do
    ...
    if Code.ensure_loaded?(Phoenix.HTML.FormData.Ecto.Changeset) do
      def encode_form_values(%{impl: Phoenix.HTML.FormData.Ecto.Changeset, source: source}, opts) do
        source |> collect_changeset_values(opts) |> CaravelaSvelte.Encoder.encode(opts)
      end
    end
  end
  ...
  def encode_form_values(form, opts) do
    ...
    CaravelaSvelte.Encoder.encode(base_values, opts)
  end
...
if Code.ensure_loaded?(Ecto.Changeset) do
  defimpl CaravelaSvelte.Encoder, for: Ecto.Changeset do
...
if Code.ensure_loaded?(Phoenix.LiveView) do
  defimpl CaravelaSvelte.Encoder, for: Phoenix.LiveView.UploadConfig do
    def encode(%Phoenix.LiveView.UploadConfig{} = struct, opts) do
      errors =
        Enum.map(struct.errors, fn {key, value} ->
          %{ref: key, error: CaravelaSvelte.Encoder.encode(value, opts)}
        end)

      entries =
        Enum.map(struct.entries, fn entry ->
          encoded = CaravelaSvelte.Encoder.encode(entry, opts)
  ...
  defimpl CaravelaSvelte.Encoder, for: Phoenix.LiveView.UploadEntry do
...
defimpl CaravelaSvelte.Encoder, for: Any do
  defmacro __deriving__(module, struct, opts) do
    fields = fields_to_encode(struct, opts)

    quote do
      defimpl CaravelaSvelte.Encoder, for: unquote(module) do
        def encode(struct, opts) do
          struct
          |> Map.take(unquote(fields))
          |> CaravelaSvelte.Encoder.encode(opts)
        end
      end
    end
  end
  ...
  # (matches @derive CaravelaSvelte.Encoder default; __meta__ stripped for Ecto schemas).
