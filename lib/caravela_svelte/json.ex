defmodule CaravelaSvelte.JSON do
  ...
  This module is the default JSON encoder for CaravelaSvelte. To use a different
  encoder like Jason, configure it in your `config.exs`:

      config :caravela_svelte, json_library: Jason
  ...
  ## Examples
      ...
      iex> CaravelaSvelte.JSON.encode!(%{foo: "bar"})
      "{\"foo\":\"bar\"}"

      iex> CaravelaSvelte.JSON.encode!([1, 2, 3])
      "[1,2,3]"
  ...
  ## Examples
      ...
      iex> CaravelaSvelte.JSON.prepare(%DateTime{} = dt)
      "2026-01-31T20:31:10Z"

      iex> CaravelaSvelte.JSON.prepare(%{notes: [%MyApp.Note{title: "Hello"}]})
      %{"notes" => [%{"title" => "Hello"}]}
