defmodule Mix.Tasks.CaravelaSvelte.Install do
  @moduledoc """
  Installer for CaravelaSvelte with Vite.

  This task first installs Vite using the PhoenixVite installer,
  then configures the project for CaravelaSvelte.
  ...
  ## Examples
      ...
      mix igniter.install caravela_svelte
      mix igniter.install caravela_svelte --bun
  ...
  # caravela_svelte as a newly-fetched dep, Igniter's BEAM files are on the path but not
  ...
  with_igniter do
    ...
    def igniter(igniter) do
      ...
      |> add_caravela_svelte_to_html_helpers(app_name)
    ...
    defp configure_environments(igniter, _app_name) do
      igniter
      |> Config.configure("config.exs", :caravela_svelte, [:ssr], true)
      |> Config.configure(
        "dev.exs",
        :caravela_svelte,
        [:ssr_module],
        {:code, Sourceror.parse_string!("CaravelaSvelte.SSR.ViteJS")}
      )
      |> Config.configure("dev.exs", :caravela_svelte, [:vite_host], "http://localhost:5173")
      |> Config.configure(
        "prod.exs",
        :caravela_svelte,
        [:ssr_module],
        {:code, Sourceror.parse_string!("CaravelaSvelte.SSR.NodeJS")}
      )
      |> Config.configure("prod.exs", :caravela_svelte, [:ssr], true)
    end

    # Add import CaravelaSvelte to html_helpers in lib/app_web.ex
    defp add_caravela_svelte_to_html_helpers(igniter, _app_name) do
      ...
      Igniter.update_file(igniter, web_file, fn source ->
        Rewrite.Source.update(source, :content, fn content ->
          if String.contains?(content, "import CaravelaSvelte") do
            content
          else
            ...
            # and html_helpers/0, causing `import CaravelaSvelte` to be injected in both.
            result =
              String.replace(
                content,
                "import Phoenix.HTML",
                "import CaravelaSvelte\n\n      import Phoenix.HTML",
                global: false
              )
    ...
    # where CaravelaSvelte.SSR.ViteJS is used instead.
        ...
        Rewrite.Source.update(source, :content, fn content ->
          if String.contains?(content, "children = [") and
            ...
            String.replace(
              ...
              ~r/([ \t]*)(children = \[)/,
              "\\1node_js_children =\n" <>
                "\\1  if Application.get_env(:caravela_svelte, :ssr_module, nil) == CaravelaSvelte.SSR.NodeJS do\n" <>
                "\\1    [{NodeJS.Supervisor, [path: CaravelaSvelte.SSR.NodeJS.server_path(), pool_size: 4]}]\n" <>
                "\\1  else\n" <>
                "\\1    []\n" <>
                "\\1  end\n\n" <>
    ...
    # Update home template with CaravelaSvelte info.
            ...
            if String.contains?(c, "/priv/svelte/"),
              do: c,
                ...
                String.trim_trailing(c) <>
                  "\n\n# CaravelaSvelte build artifacts\n/assets/svelte/_build/\n/priv/svelte/\n"
          end)
        end)
      end)
    end
    ...
    defp demo_svelte_content do
      ...
      <div class="card">
        <div class="badge">CaravelaSvelte</div>
    ...
    def run(_argv) do
      Mix.shell().error("""
      The task 'caravela_svelte.install' requires igniter. Please install igniter and try again.
